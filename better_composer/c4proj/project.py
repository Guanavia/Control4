...
class Project:
    ...
    def resolve(self, item_id: str) -> Optional[ResolvedApi]:
        it = self.get(item_id)
        return self.drivers.resolve(it.driver) if it.driver else None
    ...
    def replace_rule(self, rule: Event, actions: list):
        try:
            ev = prog.replace_event_actions(self.model, rule, actions)
        except ValueError as e:
            raise ProjectError(str(e))
        self._touch()
        return ev

    def remove_rule(self, rule: Event) -> None:
        if not prog.remove_event_handler(self.model, rule):
